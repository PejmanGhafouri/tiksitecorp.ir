...
class Comment(models.Model):
	...
	def save(self, *args, **kwargs):
		if self.article and self.user and self.content and not self.author and not self.answer and not self.status:
			email = EmailMessage(
				"دیدگاه جدید",
				"<p style='direction: rtl;text-align: right;'>سلام، دیدگاه جدیدی برای مقاله ات هست: <a href='https://tiksitesorp.ir/admin/article/comment/?article__author__id__exact={}&status__exact=0'>مشاهده</a></p>".format(self.article.author.pk),
			...
			email = EmailMessage(
				"دیدگاه شما تایید شد (:",
				"<p style='direction: rtl;text-align: right;'>سلام، دیدگاه شما که مدتی قبل برامون نوشته بودید، تایید شد: <a href='https://tiksitecorp.ir{}'>مشاهده</a><br>ممنون از دیدگاهتون</p>".format(reverse("article:detail", kwargs={'slug': self.article.slug})),
				to=[self.user.email]
			)
			email.content_subtype = "html"
			email.send()
			...
			email = EmailMessage(
				...
				"<p style='direction: rtl;text-align: right;'>سلام، دیدگاه شما که مدتی قبل برامون نوشته بودید، تایید شد و بهش پاسخ دادیم: <a href='https://tiksitecorp.ir{}'>مشاهده</a><br>ممنون از دیدگاهتون</p>".format(reverse("article:detail", kwargs={'slug': self.article.slug})),
				to=[self.user.email]
			)
			email.content_subtype = "html"
			email.send()
